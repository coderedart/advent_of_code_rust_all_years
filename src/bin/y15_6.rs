...
impl Grid {
    fn parse_from_str(input: &str) -> Grid {
        let mut input = input.split_whitespace();
        let mut start = input.next().unwrap().split(',');
        let _ = input.next();
        let mut end = input.next().unwrap().split(',');
...
fn parse_action_grid(input: &str) -> (Action, Grid) {
    let grid_input;
    let action = if input.starts_with("turn on") {
        grid_input = &input[8..];
        Action::On
    ...
    (action, Grid::parse_from_str(grid_input))
}
fn main() {
    {
        let mut lights = [[false; 1000]; 1000];
        for input in INPUT.lines() {
            let (a, g) = parse_action_grid(input);
            for row in lights[g.start_y..=g.end_y].as_mut() {
                for light in row[g.start_x..=g.end_x].as_mut() {
                    match a {
    ...
    {
        let mut lights = Box::new([[0u32; 1000]; 1000]);
        for input in INPUT.lines() {
            let (a, g) = parse_action_grid(input);
            for row in lights[g.start_y..=g.end_y].as_mut() {
                for light in row[g.start_x..=g.end_x].as_mut() {
                    match a {
                        Action::On => *light = light.saturating_add(1),
...
const INPUT: &str = r#"turn on 489,959 through 759,964
